...
latest_panel = panel[panel["snapshot_dt"] == latest].copy()
expected_scored = int((latest_panel["score_eligible_cat_14"] == 1).sum())
...
value_counts = rollup["value_band"].astype(str).value_counts()
if len(rollup) >= 3:
    tercile_spread = int(value_counts.max() - value_counts.min())
else:
    tercile_spread = 0

checks = {
    "scores_nonempty": bool(len(scores) > 0),
    "scores_match_latest_eligibility": bool(len(scores) == expected_scored),
    "scores_key_unique": bool(int(scores.duplicated(CATEGORY_KEYS).sum()) == 0),
    "probability_in_range": bool(scores["p_14"].between(0, 1).all()),
    "risk_tiers_match_thresholds": bool(
        (scores["risk_tier"].astype(str) == expected_tier).all()
    ),
    ...
    "value_bands_valid": bool(
        set(rollup["value_band"].dropna().astype(str))
        <= {"Low", "Mid", "High"}
    ),
    "value_bands_tercile_balanced": bool(tercile_spread <= 1),
    ...
    "scored_plus_rejected_reconciles": bool(
        len(scores) + len(reject) == len(latest_panel)
    ),
}
...
result = {
    ...
    "rows": {
        "latest_panel": int(len(latest_panel)),
        "expected_score_eligible": expected_scored,
    ...
    "value_band_counts": {
        str(k): int(v)
        for k, v in value_counts.sort_index().items()
    },
}
